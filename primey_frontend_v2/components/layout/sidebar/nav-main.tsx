      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            ...
            icon: Stethoscope,
            permission: PERMISSIONS.SYSTEM_ACTIVITY_PROFILES_VIEW,
            workspaces: ["system"],
          },
        ],
      },
      ...
      },
      {
        ...
        items: [
          ...
          },
          {
            title: {
              ar: "تكامل MhamCloud",
              en: "MhamCloud Integration",
            },
            href: "/system/integrations/mhamcloud",
            icon: Database,
            permission: PERMISSIONS.SYSTEM_INTEGRATIONS_VIEW,
            workspaces: ["system"],
          },
...
/* BUNDUI SEARCH ADAPTER — derived only from Mhamcloud canonical nav data. */
export const navItems = systemNavItems.map((group) => ({
  title: group.title.ar,
  items: group.items.map((item) => ({
    title: item.title.ar,
    href: item.href,
    icon: item.icon,
    items: item.items?.map((child) => ({
      title: child.title.ar,
      href: child.href,
      icon: child.icon,
      items: child.items?.map((grandChild) => ({
        title: grandChild.title.ar,
...
function getStoredLocale(): AppLocale {
  try {
    ...
    const savedLocale = window.localStorage.getItem("Mhamcloud-locale");
...
export function NavMain({ type }: NavMainProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("Mhamcloud-locale-changed", syncLocaleAfterPaint);
    window.addEventListener("storage", syncLocaleAfterPaint);

    return () => {
      window.removeEventListener("Mhamcloud-locale-changed", syncLocaleAfterPaint);
  ...
  const renderIconWrap = (
    ...
    if (!Icon) return null;
    return <Icon className={cn(level === 0 ? "size-4" : "size-3.5", "shrink-0")} />;
  };
