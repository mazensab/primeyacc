...
const API_ENDPOINT = API_PATHS.systemNotifications.list;
...
export function SystemNotificationsCenter({ mode = "overview" }: { mode?: NotificationsPageMode }) {
  ...
  async function markAllRead() {
    ...
    try {
      setSaving(true);
      await postJson(makeApiUrl(API_PATHS.systemNotifications.readAll));
  ...
  async function toggleRead(item: NotificationRecord) {
    try {
      ...
      const path = item.isRead
        ? API_PATHS.systemNotifications.markUnread(item.id)
        : API_PATHS.systemNotifications.markRead(item.id);
  ...
  return (
    <div dir={dir} className="space-y-4 lg:space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4 lg:gap-6">
        <SystemMetricCard title={t.totalNotifications} value={stats.total} description={t.fromLiveApi} icon={Bell} />
        <SystemMetricCard title={t.unreadNotifications} value={stats.unread} description={t.fromLiveApi} icon={AlertTriangle} />
        <SystemMetricCard title={t.readNotifications} value={stats.read} description={t.fromLiveApi} icon={CheckCircle2} />
        <SystemMetricCard title={t.companyWideNotifications} value={stats.companyWide} description={t.fromLiveApi} icon={Building2} />
      </div>

      <Card className="w-full">
        <CardHeader>
          <CardTitle icon={Bell}>{t.tableTitle}</CardTitle>
        ...
        <CardContent className="space-y-4">
          <DataRegisterToolbar className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            <div className="flex min-w-0 flex-1 flex-col gap-2 md:flex-row md:flex-wrap md:items-center">
              ...
              <Select value={channel} onValueChange={(value) => setChannel(value as ChannelFilter)}>
                <SelectTrigger className="h-9 bg-background shadow-none md:w-[165px]">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <Table className="min-w-[1060px] table-fixed">
              <TableHeader>
                <TableRow className="h-11 bg-muted/40 hover:bg-muted/40">
                  <TableHead className={cn("h-11 w-[260px] px-4 text-xs font-semibold text-muted-foreground", alignClass)}>{t.notification}</TableHead>
                  <TableHead className={cn("h-11 w-[170px] px-4 text-xs font-semibold text-muted-foreground", alignClass)}>{t.company}</TableHead>
                  <TableHead className={cn("h-11 w-[150px] px-4 text-xs font-semibold text-muted-foreground", alignClass)}>{t.recipient}</TableHead>
                  <TableHead className={cn("h-11 w-[115px] px-4 text-xs font-semibold text-muted-foreground", alignClass)}>{t.channel}</TableHead>
                  <TableHead className={cn("h-11 w-[115px] px-4 text-xs font-semibold text-muted-foreground", alignClass)}>{t.priority}</TableHead>
                  <TableHead className={cn("h-11 w-[115px] px-4 text-xs font-semibold text-muted-foreground", alignClass)}>{t.status}</TableHead>
                  <TableHead className={cn("h-11 w-[130px] px-4 text-xs font-semibold text-muted-foreground", alignClass)}>{t.createdAt}</TableHead>
                  <TableHead className="sticky end-0 z-10 h-11 w-[145px] bg-muted/40 px-3 text-center text-xs font-semibold text-muted-foreground">{t.open}</TableHead>
                </TableRow>
              </TableHeader>
                    ...
                    return (
                      <TableRow key={item.id} className="h-[68px]">
                        ...
                        <TableCell className={cn("h-[68px] px-4 align-middle", alignClass)}>
                          <Badge variant="secondary" className="rounded-full">{getChannelLabel(item.channel, locale)}</Badge>
                        </TableCell>

                        <TableCell className={cn("h-[68px] px-4 align-middle", alignClass)}>
                          <Badge variant="outline" className="rounded-full">{getPriorityLabel(item.priority, locale)}</Badge>
                        </TableCell>
          ...
          <DataRegisterResultCount
            ...
            ofLabel={t.of}
            totalCount={formatInteger(apiTotal || notifications.length)}
            rowsLabel={t.rows}
          />
        </CardContent>
      </Card>
