...
export function EventDialog({ event, isOpen, onClose, onSave, onDelete }: EventDialogProps) {
  ...
  const [endTime, setEndTime] = useState(`${String(DefaultEndHour).padStart(2, "0")}:00`);
  ...
  return (
    <Drawer direction="right" open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DrawerContent className="w-full gap-0 sm:max-w-md max-sm:w-[92%]!">
        <DrawerHeader className="border-b">
          <DrawerTitle>{event?.id ? "Edit Event" : "Create Event"}</DrawerTitle>
          <DrawerDescription className="sr-only">
            {event?.id ? "Edit the details of this event" : "Add a new event to your calendar"}
          </DrawerDescription>
        </DrawerHeader>
        {error && (
          <div className="bg-destructive/15 text-destructive mx-4 mt-4 rounded-md px-3 py-2 text-sm">
            {error}
          </div>
        )}
        <div className="grid gap-4 overflow-y-auto p-4">
          ...
          <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-4">
            <div className="min-w-0 flex-1 *:not-first:mt-1.5">
        ...
        <DrawerFooter className="flex-row border-t sm:justify-between">
          {event?.id && (
