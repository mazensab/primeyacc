...
export default function SystemCompanyUserCreatePage() {
  ...
  React.useEffect(() => {
    ...
    async function loadCompany() {
      ...
      try {
        const response = await fetch(makeApiUrl(`/api/system/companies/${companyId}/`), {
          method: "GET",
          credentials: "include",
          cache: "no-store",
          headers: {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch(
        makeApiUrl(`/api/system/companies/${companyId}/users/create/`),
        {
          method: "POST",
          credentials: "include",
          headers: {
    ...
    <main dir={dir} className="w-full space-y-4 text-foreground lg:space-y-6">
      <div className="w-full space-y-4 lg:space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {companyLoading
                ? t.companyLoading
                : company?.companyCode || company?.id || companyId}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {companyLoading ? (
        ...
        >
          <Card>
            <CardHeader>
              <CardTitle icon={UserPlus} iconPosition="opposite">
                {t.accountData}
              </CardTitle>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t.membershipDesc}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-5">
              <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
                <div>
                  ...
                  <Select
                    value={form.role}
                    onValueChange={(value) => updateField("role", value as RoleValue)}
                  >
              ...
              <label className="block text-sm font-medium">
                <span className="mb-2 block">{t.notes}</span>
                <textarea
                  value={form.notes}
                  rows={5}
                  onChange={(event) => updateField("notes", event.target.value)}
                  className="w-full resize-y rounded-lg border border-input bg-background px-3 py-3 text-sm outline-none transition placeholder:text-muted-foreground focus-visible:ring-2 focus-visible:ring-ring"
                />
              </label>
