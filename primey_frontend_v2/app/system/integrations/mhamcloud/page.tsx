...
export default function SystemMhamCloudIntegrationPage() {
  ...
  async function openDetail(row:CompanySync) {
    setBusy(`detail:${row.business_id}`);
    try {
      const payload=await request<{ok:boolean;data:CompanySync}>(API_PATHS.systemMhamCloud.companyDetail(row.business_id));
  ...
  if(loading)return <main className="space-y-4 lg:space-y-6"><Skeleton className="h-24"/><Skeleton className="h-[600px]"/></main>;
  ...
  return <main dir={locale==="ar"?"rtl":"ltr"} className="space-y-4 text-foreground lg:space-y-6">
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <SystemMetricCard title={t.total} value={status?.companies.total||companies.length} description={t.companiesDesc} icon={Database}/>
      <SystemMetricCard title={t.synced} value={synced} description={t.companiesDesc} icon={CheckCircle2}/>
      <SystemMetricCard title={t.unchanged} value={status?.companies.unchanged||0} description={t.companiesDesc} icon={ShieldCheck}/>
      <SystemMetricCard title={t.failed} value={status?.companies.failed||0} description={t.companiesDesc} icon={XCircle}/>
      <SystemMetricCard title={t.changed} value={status?.latest_state?.changed_count||0} description={date(status?.latest_state?.completed_at)} icon={Activity}/>
      <SystemMetricCard title={t.connection} value={connection} description={date(status?.connection.last_test_at)} icon={PlugZap}/>
      <SystemMetricCard title={t.requested} value={status?.latest_state?.requested_count||0} description={date(status?.latest_state?.started_at)} icon={FileClock}/>
      <SystemMetricCard title={t.failures} value={status?.latest_state?.failure_count||0} description={date(status?.latest_state?.completed_at)} icon={AlertTriangle}/>
    </div>

    <Card className="gap-0 rounded-lg border bg-card py-0 shadow-none"><CardHeader className="border-b py-5"><div className="flex items-start justify-between gap-3"><div><CardTitle icon={PlugZap}>{t.connection}</CardTitle><CardDescription>{settings?.base_url||form.base_url}</CardDescription></div><div className="flex gap-2">{badge(status?.connection.last_test_ok===true?"SUCCESS":status?.connection.last_test_ok===false?"FAILED":"UNKNOWN")}<Button size="sm" variant="outline" className={registerOutlineButtonClass} onClick={()=>void testConnection()}><PlugZap className="h-4 w-4"/>{t.test}</Button></div></div></CardHeader><CardContent className="grid gap-4 py-5 sm:grid-cols-2 lg:grid-cols-4">{[[t.clientId,settings?.client_id_configured],[t.clientSecret,settings?.client_secret_configured],[t.username,settings?.username_configured],[t.password,settings?.password_configured]].map(([label,ok])=><div key={String(label)} className="rounded-lg border bg-muted/20 p-4"><p className="font-semibold">{String(label)}</p><p className="mt-1 text-sm text-muted-foreground">{ok?t.configured:t.missing}</p></div>)}</CardContent></Card>

    <Card className="gap-0 overflow-hidden rounded-lg border bg-card py-0 shadow-none">
      <CardHeader className="border-b py-5"><div className="space-y-4"><div className="flex justify-between gap-3"><div><CardTitle icon={Database}>{t.companies}</CardTitle><CardDescription>{t.companiesDesc}</CardDescription></div><div className="flex gap-2"><Button size="sm" className={registerBrandButtonClass} onClick={excel}><Download className="h-4 w-4"/>{t.excel}</Button><Button size="sm" className={registerBrandButtonClass} onClick={print}><Printer className="h-4 w-4"/>{t.print}</Button></div></div>
      ...
      {visible.length?<DataRegisterTableFrame className="rounded-none border-x-0"><div className="overflow-x-auto"><Table className="min-w-[1200px] table-fixed"><TableHeader><TableRow><TableHead className="sticky start-0 z-20 bg-background">{t.company}</TableHead><TableHead>{t.business}</TableHead><TableHead>{t.primey}</TableHead><TableHead>{t.status}</TableHead><TableHead>{t.progress}</TableHead><TableHead>{t.last}</TableHead><TableHead>{t.error}</TableHead><TableHead className="sticky end-0 z-20 bg-background">{t.actions}</TableHead></TableRow></TableHeader><TableBody>{visible.map(r=>{const p=r.domain_count?Math.round(r.successful_domain_count/r.domain_count*100):(r.status==="FAILED"?0:100);return <TableRow key={r.business_id} className="h-[68px] cursor-pointer" onDoubleClick={()=>void openDetail(r)}><TableCell className="sticky start-0 z-10 bg-background"><p className="max-w-[250px] truncate font-semibold">{r.company_name||"—"}</p></TableCell><TableCell dir="ltr">{r.business_id}</TableCell><TableCell dir="ltr">{r.company_id??"—"}</TableCell><TableCell>{badge(r.status)}</TableCell><TableCell><div className="w-[150px]"><div className="mb-1 flex justify-between text-xs text-muted-foreground"><span>{r.domain_count?`${r.successful_domain_count}/${r.domain_count}`:"—"}</span><span>{p}%</span></div><Progress value={p} className="h-1.5"/></div></TableCell><TableCell dir="ltr">{date(r.last_attempt_at)}</TableCell><TableCell><p className={r.safe_error_message?"max-w-[300px] truncate text-red-600":"max-w-[300px] truncate text-muted-foreground"} title={r.safe_error_message||t.noError}>{r.safe_error_message||t.noError}</p></TableCell><TableCell className="sticky end-0 z-10 bg-background"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon"><MoreVertical className="h-4 w-4"/></Button></DropdownMenuTrigger><DropdownMenuContent><DropdownMenuItem onClick={()=>void openDetail(r)}><Eye className="h-4 w-4"/>{t.details}</DropdownMenuItem><DropdownMenuSeparator/><DropdownMenuItem disabled={busy===r.business_id||Boolean(status?.sync_running)} onClick={()=>void retry(r)}><RefreshCcw className="h-4 w-4"/>{t.retry}</DropdownMenuItem></DropdownMenuContent></DropdownMenu></TableCell></TableRow>})}</TableBody></Table></div></DataRegisterTableFrame>:<DataRegisterEmptyState title={t.noRows} description={t.noRowsDesc} icon={SearchX}/>}
      <div className="px-(--card-spacing) pb-(--card-spacing) pt-3"><DataRegisterResultCount showingLabel={locale==="ar"?"عرض":"Showing"} showingCount={visible.length} ofLabel={locale==="ar"?"من":"of"} totalCount={companies.length} rowsLabel={locale==="ar"?"شركة":"companies"} /></div>
    </Card>

    <Card className="gap-0 overflow-hidden rounded-lg border bg-card py-0 shadow-none"><CardHeader className="border-b py-5"><CardTitle icon={Clock3}>{t.runs}</CardTitle></CardHeader>{runs.length?<DataRegisterTableFrame className="rounded-none border-x-0"><div className="overflow-x-auto"><Table className="min-w-[1000px] table-fixed"><TableHeader><TableRow><TableHead>Run ID</TableHead><TableHead>{t.trigger}</TableHead><TableHead>{t.status}</TableHead><TableHead>{t.started}</TableHead><TableHead>{t.finished}</TableHead><TableHead>{t.requested}</TableHead><TableHead>{t.changed}</TableHead><TableHead>{t.failures}</TableHead></TableRow></TableHeader><TableBody>{runs.slice(0,100).map(r=><TableRow key={r.id} className="h-[68px]"><TableCell dir="ltr" className="font-mono text-xs">{r.id}</TableCell><TableCell>{r.trigger}</TableCell><TableCell>{badge(r.status)}</TableCell><TableCell dir="ltr">{date(r.started_at||r.queued_at)}</TableCell><TableCell dir="ltr">{date(r.completed_at)}</TableCell><TableCell>{r.requested_count??0}</TableCell><TableCell>{r.changed_count??0}</TableCell><TableCell>{r.failure_count??0}</TableCell></TableRow>)}</TableBody></Table></div></DataRegisterTableFrame>:<DataRegisterEmptyState title={t.noRuns} description={t.noRuns} icon={Clock3}/>}</Card>
