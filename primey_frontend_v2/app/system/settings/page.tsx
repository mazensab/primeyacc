...
export default function SystemSettingsPage() {
  ...
  if (error) {
    return (
      <main dir={dir} className="bg-transparent text-foreground">
      ...
      <div className="w-full space-y-4 lg:space-y-6">
        <div className="flex flex-row items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            <Button variant="outline" className={registerOutlineButtonClass} onClick={() => void loadSettings({ silent: true })} disabled={refreshing}>
              {refreshing ? <Loader2 className="animate-spin" /> : <RefreshCw />}<span className="hidden lg:inline">{t.refresh}</span>
            </Button>
            <Button variant="outline" className={registerOutlineButtonClass} onClick={() => void seedDefaults()} disabled={seeding}>
              {seeding ? <Loader2 className="animate-spin" /> : <DatabaseZap />}<span className="hidden lg:inline">{seeding ? t.seeding : t.seedDefaults}</span>
            </Button>
            <Button variant="outline" className={registerOutlineButtonClass} onClick={exportExcel}><FileSpreadsheet /><span className="hidden lg:inline">{t.exportExcel}</span></Button>
            <Button className={registerBrandButtonClass} onClick={() => openPrintWindow("print")}><Printer /><span className="hidden lg:inline">{t.print}</span></Button>
          </div>
        </div>

        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <SystemMetricCard title={t.totalSettings} value={stats.total} description={t.fromLiveApi} icon={Settings} />
          <SystemMetricCard title={t.activeSettings} value={stats.active} description={t.fromLiveApi} icon={CheckCircle2} />
          <SystemMetricCard title={t.publicSettings} value={stats.public} description={t.fromLiveApi} icon={ShieldCheck} />
          <SystemMetricCard title={t.requiredSettings} value={stats.required} description={t.fromLiveApi} icon={Layers3} />
        </div>
        ...
        <Card className="w-full rounded-lg border bg-card shadow-none">
          <CardHeader className="gap-3">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
              <div>
                <CardTitle icon={Settings}>{t.tableTitle}</CardTitle>
                  ...
                  <TableBody>
                    {previewRows.length ? (
                      previewRows.map((setting) => {
                        ...
                        return (
                          <TableRow key={setting.id || `${setting.group}:${setting.key}`} className="h-[68px]">
                            ...
                            <TableCell className={cn(actionStickyClass, "z-10 h-[68px] bg-background px-3 text-center align-middle")}>
                              <Button
                                variant="outline"
                                size="sm"
