...
export function normalizeWorkspace(workspace: unknown): string {
  ...
  if (value === "company" || value === "center" || value === "provider") {
    return "company";
  }
...
export function getDashboardPath(session?: PermissionSession | null): string {
  ...
  if (role === ROLES.PROVIDER_ADMIN || workspace === "company") {
    return "/company";
  }
...
export function normalizePath(pathname: string): string {
  ...
  if (cleanPath === "/provider" || cleanPath === "/center" || cleanPath === "/company") {
    return "/company";
  }

  if (cleanPath.startsWith("/provider/")) {
    return cleanPath.replace("/provider", "/company");
  }

  if (cleanPath.startsWith("/center/")) {
    return cleanPath.replace("/center", "/company");
  }
