...
# V2-24A EXTENDED SYSTEM TEMPLATE CATALOG
def _v224a_template(code,name_ar,name_en,category,body_ar,body_en,variables,event,module):
    return {"code":code,"name_ar":name_ar,"name_en":name_en,"category":category,"body_ar":body_ar,"body_en":body_en,"variables":variables,"metadata":{"scope":"SYSTEM","event":event,"module":module,"i18n":{"ar":{"name":name_ar,"body":body_ar},"en":{"name":name_en,"body":body_en}}}}

SYSTEM_WHATSAPP_READY_TEMPLATES.extend([
    _v224a_template('SYSTEM_COMPANY_SUSPENDED','تعليق الشركة','Company suspended','CUSTOMER_SERVICE','تم تعليق شركة {{company_name}} مؤقتًا. السبب: {{reason}}.','Company {{company_name}} has been suspended. Reason: {{reason}}.',['company_name', 'reason'],'company.suspended','companies'),
    _v224a_template('SYSTEM_COMPANY_REACTIVATED','إعادة تفعيل الشركة','Company reactivated','CUSTOMER_SERVICE','تمت إعادة تفعيل شركة {{company_name}} بنجاح. رابط الدخول: {{login_url}}','Company {{company_name}} has been reactivated. Sign in: {{login_url}}',['company_name', 'login_url'],'company.reactivated','companies'),
    _v224a_template('SYSTEM_USER_CREATED','إضافة مستخدم جديد','User created','CUSTOMER_SERVICE','مرحبًا {{user_name}}، تم إنشاء حسابك في شركة {{company_name}} بدور {{role_name}}. رابط الدخول: {{login_url}}','Hello {{user_name}}, your account at {{company_name}} was created with role {{role_name}}. Sign in: {{login_url}}',['user_name', 'company_name', 'role_name', 'login_url'],'user.created','users'),
    _v224a_template('SYSTEM_USER_SUSPENDED','تعليق المستخدم','User suspended','CUSTOMER_SERVICE','مرحبًا {{user_name}}، تم تعليق حسابك في شركة {{company_name}}. السبب: {{reason}}.','Hello {{user_name}}, your account at {{company_name}} was suspended. Reason: {{reason}}.',['user_name', 'company_name', 'reason'],'user.suspended','users'),
    _v224a_template('SYSTEM_USER_REACTIVATED','إعادة تفعيل المستخدم','User reactivated','CUSTOMER_SERVICE','مرحبًا {{user_name}}، تمت إعادة تفعيل حسابك في شركة {{company_name}}. رابط الدخول: {{login_url}}','Hello {{user_name}}, your account at {{company_name}} was reactivated. Sign in: {{login_url}}',['user_name', 'company_name', 'login_url'],'user.reactivated','users'),
    _v224a_template('SYSTEM_USER_ROLE_CHANGED','تغيير دور المستخدم','User role changed','CUSTOMER_SERVICE','مرحبًا {{user_name}}، تم تغيير دورك في شركة {{company_name}} من {{old_role_name}} إلى {{new_role_name}}.','Hello {{user_name}}, your role at {{company_name}} changed from {{old_role_name}} to {{new_role_name}}.',['user_name', 'company_name', 'old_role_name', 'new_role_name'],'user.role_changed','users'),
    _v224a_template('SYSTEM_PASSWORD_CHANGED','تغيير كلمة المرور','Password changed','CUSTOMER_SERVICE','مرحبًا {{user_name}}، تم تغيير كلمة مرور حسابك بتاريخ {{event_time}}. إذا لم تقم بذلك فتواصل مع الدعم فورًا.','Hello {{user_name}}, your password was changed at {{event_time}}. If this was not you, contact support immediately.',['user_name', 'event_time'],'user.password_changed','auth'),
    _v224a_template('SYSTEM_SUBSCRIPTION_GRACE_STARTED','بدء فترة السماح','Subscription grace period started','CUSTOMER_SERVICE','بدأت فترة السماح لاشتراك شركة {{company_name}} على باقة {{plan_name}} وتنتهي في {{grace_end_date}}.','The grace period for {{company_name}} on {{plan_name}} ends on {{grace_end_date}}.',['company_name', 'plan_name', 'grace_end_date'],'subscription.grace_started','subscriptions'),
    _v224a_template('SYSTEM_SUBSCRIPTION_SUSPENDED','تعليق الاشتراك','Subscription suspended','CUSTOMER_SERVICE','تم تعليق اشتراك شركة {{company_name}} على باقة {{plan_name}}. السبب: {{reason}}.','The subscription for {{company_name}} on {{plan_name}} was suspended. Reason: {{reason}}.',['company_name', 'plan_name', 'reason'],'subscription.suspended','subscriptions'),
    _v224a_template('SYSTEM_SUBSCRIPTION_REACTIVATED','إعادة تفعيل الاشتراك','Subscription reactivated','CUSTOMER_SERVICE','تمت إعادة تفعيل اشتراك شركة {{company_name}} على باقة {{plan_name}}. تاريخ الانتهاء: {{end_date}}.','The subscription for {{company_name}} on {{plan_name}} was reactivated. Expiry: {{end_date}}.',['company_name', 'plan_name', 'end_date'],'subscription.reactivated','subscriptions'),
    _v224a_template('SYSTEM_SUBSCRIPTION_PLAN_CHANGED','تغيير باقة الاشتراك','Subscription plan changed','CUSTOMER_SERVICE','تم تغيير اشتراك شركة {{company_name}} من {{old_plan_name}} إلى {{new_plan_name}} اعتبارًا من {{effective_date}}.','The subscription for {{company_name}} changed from {{old_plan_name}} to {{new_plan_name}} effective {{effective_date}}.',['company_name', 'old_plan_name', 'new_plan_name', 'effective_date'],'subscription.plan_changed','subscriptions'),
    _v224a_template('SYSTEM_RENEWAL_PAYMENT_REQUIRED','مطلوب سداد للتجديد','Renewal payment required','ACCOUNTING','لتجديد اشتراك شركة {{company_name}} على باقة {{plan_name}} يرجى سداد {{amount}} {{currency}} قبل {{due_date}}.','To renew {{company_name}} on {{plan_name}}, pay {{amount}} {{currency}} before {{due_date}}.',['company_name', 'plan_name', 'amount', 'currency', 'due_date'],'subscription.renewal_payment_required','subscriptions'),
    _v224a_template('SYSTEM_RENEWAL_FAILED','فشل تجديد الاشتراك','Subscription renewal failed','CUSTOMER_SERVICE','تعذر تجديد اشتراك شركة {{company_name}} على باقة {{plan_name}}. السبب: {{failure_reason}}.','Renewal for {{company_name}} on {{plan_name}} failed. Reason: {{failure_reason}}.',['company_name', 'plan_name', 'failure_reason'],'subscription.renewal_failed','subscriptions'),
    _v224a_template('SYSTEM_INVOICE_DUE_SOON','قرب استحقاق الفاتورة','Invoice due soon','ACCOUNTING','تذكير: الفاتورة {{invoice_number}} لشركة {{company_name}} بقيمة {{amount}} {{currency}} تستحق في {{due_date}}.','Reminder: invoice {{invoice_number}} for {{company_name}}, {{amount}} {{currency}}, is due on {{due_date}}.',['invoice_number', 'company_name', 'amount', 'currency', 'due_date'],'invoice.due_soon','billing'),
    _v224a_template('SYSTEM_INVOICE_OVERDUE','فاتورة متأخرة','Invoice overdue','ACCOUNTING','الفاتورة {{invoice_number}} لشركة {{company_name}} بقيمة {{amount}} {{currency}} متأخرة منذ {{due_date}}.','Invoice {{invoice_number}} for {{company_name}}, {{amount}} {{currency}}, is overdue since {{due_date}}.',['invoice_number', 'company_name', 'amount', 'currency', 'due_date'],'invoice.overdue','billing'),
    _v224a_template('SYSTEM_INVOICE_PAID','سداد الفاتورة','Invoice paid','ACCOUNTING','تم سداد الفاتورة {{invoice_number}} لشركة {{company_name}} بمبلغ {{amount}} {{currency}} بنجاح.','Invoice {{invoice_number}} for {{company_name}} was paid successfully: {{amount}} {{currency}}.',['invoice_number', 'company_name', 'amount', 'currency'],'invoice.paid','billing'),
    _v224a_template('SYSTEM_RECEIPT_ISSUED','إصدار سند قبض','Receipt issued','ACCOUNTING','تم إصدار سند قبض رقم {{receipt_number}} لشركة {{company_name}} بمبلغ {{amount}} {{currency}}. الرابط: {{receipt_url}}','Receipt {{receipt_number}} was issued for {{company_name}} for {{amount}} {{currency}}. Link: {{receipt_url}}',['receipt_number', 'company_name', 'amount', 'currency', 'receipt_url'],'receipt.issued','billing'),
    _v224a_template('SYSTEM_PAYMENT_CANCELLED','إلغاء عملية الدفع','Payment cancelled','TREASURY','تم إلغاء عملية الدفع لشركة {{company_name}} بمبلغ {{amount}} {{currency}}. المرجع: {{payment_reference}}.','Payment for {{company_name}}, {{amount}} {{currency}}, was cancelled. Reference: {{payment_reference}}.',['company_name', 'amount', 'currency', 'payment_reference'],'payment.cancelled','payments'),
    _v224a_template('SYSTEM_REFUND_SUCCEEDED','نجاح استرداد المبلغ','Refund succeeded','TREASURY','تم استرداد مبلغ {{refund_amount}} {{currency}} لشركة {{company_name}} بنجاح. مرجع الاسترداد: {{refund_reference}}.','Refund of {{refund_amount}} {{currency}} for {{company_name}} succeeded. Reference: {{refund_reference}}.',['refund_amount', 'currency', 'company_name', 'refund_reference'],'refund.succeeded','payments'),
    _v224a_template('SYSTEM_REFUND_FAILED','فشل استرداد المبلغ','Refund failed','TREASURY','تعذر استرداد مبلغ {{refund_amount}} {{currency}} لشركة {{company_name}}. السبب: {{failure_reason}}.','Refund of {{refund_amount}} {{currency}} for {{company_name}} failed. Reason: {{failure_reason}}.',['refund_amount', 'currency', 'company_name', 'failure_reason'],'refund.failed','payments'),
    _v224a_template('SYSTEM_MAINTENANCE_SCHEDULED','صيانة مجدولة','Scheduled maintenance','GENERAL','تنبيه: توجد صيانة مجدولة للنظام من {{start_time}} إلى {{end_time}}. التفاصيل: {{maintenance_note}}','Notice: scheduled maintenance is planned from {{start_time}} to {{end_time}}. Details: {{maintenance_note}}',['start_time', 'end_time', 'maintenance_note'],'system.maintenance_scheduled','system'),
    _v224a_template('SYSTEM_SERVICE_RESTORED','عودة الخدمة','Service restored','GENERAL','تمت استعادة خدمة {{service_name}} وأصبحت متاحة بشكل طبيعي اعتبارًا من {{restored_at}}.','Service {{service_name}} has been restored and is operating normally as of {{restored_at}}.',['service_name', 'restored_at'],'system.service_restored','system'),
])
