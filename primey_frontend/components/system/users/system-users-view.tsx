...
export function SystemUsersView({ mode = "overview" }: { mode?: "overview" | "list" }) {
    ...
    async ({ silent = false }: { silent?: boolean } = {}) => {
      try {
        ...
        while (page <= 1000) {
          ...
          const pageRows = extractArray(payload).map(normalizeUser);
          const payloadRecord = asRecord(payload);
          apiCount = Math.max(apiCount, extractCount(payload));

          allRows.push(...pageRows);

          const hasNext =
            payloadRecord.has_next === true ||
            (pageRows.length > 0 && allRows.length < apiCount);
  ...
  function exportExcel() {
    ...
    downloadExcelReport({
      locale,
      title: t.reportTitle,
      filename: `Mhamcloud-components-system-users-system-users-view-${new Date().toISOString().slice(0, 10)}.xls`,
