...
function Kpi({ title, value }: { title: string; value: React.ReactNode }) {
  return <Card className="rounded-2xl border-border/70 shadow-sm"><CardHeader className="pb-2"><CardDescription className="font-semibold text-foreground">{title}</CardDescription><CardTitle className="mt-2 text-2xl tabular-nums">{value}</CardTitle></CardHeader></Card>;
}
function Header({ mode, locale, refreshing, refresh }: { mode: PlatformBillingDocumentsMode; locale: Locale; refreshing: boolean; refresh: () => void }) {
  ...
  return <section className="overflow-hidden rounded-lg border bg-card shadow-none"><div className="relative p-6 sm:p-8"><div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between"><div className="max-w-4xl"><div className="mb-3 inline-flex items-center gap-2 rounded-full border bg-background px-3 py-1 text-xs font-medium text-muted-foreground"><Sparkles className="h-3.5 w-3.5 text-primary" />{t.badge}</div><h1 className="text-3xl font-bold tracking-tight sm:text-4xl">{title}</h1><p className="mt-3 text-sm leading-7 text-muted-foreground sm:text-base">{desc}</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" className="h-9 rounded-lg bg-background shadow-none" onClick={refresh} disabled={refreshing}>{refreshing ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}{t.refresh}</Button><Button asChild variant="outline" className="h-9 rounded-lg bg-background shadow-none"><Link href="/system/invoices"><ReceiptText className="h-4 w-4" />{t.overview}</Link></Button><Button asChild variant="outline" className="h-9 rounded-lg bg-background shadow-none"><Link href="/system/invoices/list"><FileText className="h-4 w-4" />{t.invoices}</Link></Button><Button asChild variant="outline" className="h-9 rounded-lg bg-background shadow-none"><Link href="/system/invoices/receipts"><ReceiptText className="h-4 w-4" />{t.receipts}</Link></Button><Button asChild variant="outline" className="h-9 rounded-lg bg-background shadow-none"><Link href="/system/platform-payments"><CreditCard className="h-4 w-4" />{t.payments}</Link></Button></div></div></div></section>;
}
function Loading() {
...
function Register({ documents, mode, locale }: { documents: BillingDocument[]; mode: "overview" | "invoices" | "receipts"; locale: Locale }) {
  ...
  function exportExcel() {
    ...
    downloadExcelHtmlReport(html, `Mhamcloud-platform-billing-${new Date().toISOString().slice(0, 10)}.xls`);
  }
  return <><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><Kpi title={t.totalDocuments} value={stats.total} /><Kpi title={t.invoiceCount} value={stats.invoices} /><Kpi title={t.receiptCount} value={stats.receipts} /><Kpi title={t.totalAmount} value={<Money amount={String(stats.amount)} currency="SAR" />} /></div><Card className="rounded-lg border bg-card shadow-none"><CardContent className="space-y-3 pt-6"><div className="flex flex-col gap-3 xl:flex-row xl:items-center"><div className="relative min-w-0 flex-1"><Search className="pointer-events-none absolute start-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder={t.search} className="h-9 rounded-lg ps-9" /></div><Select value={status} onValueChange={setStatus}><SelectTrigger className="h-9 rounded-lg xl:w-[150px]"><SelectValue placeholder={t.status} /></SelectTrigger><SelectContent><SelectItem value="all">{t.all}</SelectItem>{statuses.map((v) => <SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent></Select><Select value={company} onValueChange={setCompany}><SelectTrigger className="h-9 rounded-lg xl:w-[200px]"><SelectValue placeholder={t.company} /></SelectTrigger><SelectContent><SelectItem value="all">{t.all}</SelectItem>{companies.map(([id, label]) => <SelectItem key={id} value={id}>{label}</SelectItem>)}</SelectContent></Select><Select value={subscription} onValueChange={setSubscription}><SelectTrigger className="h-9 rounded-lg xl:w-[210px]"><SelectValue placeholder={t.subscription} /></SelectTrigger><SelectContent><SelectItem value="all">{t.all}</SelectItem>{subscriptions.map(([id, label]) => <SelectItem key={id} value={id}>{label}</SelectItem>)}</SelectContent></Select></div><div className="flex flex-wrap gap-2"><Popover><PopoverTrigger asChild><Button variant="outline" className="h-9 rounded-lg"><CalendarDays className="h-4 w-4" />{t.from}: {fromDate || "—"}</Button></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={parseIsoDate(fromDate)} onSelect={(d) => setFromDate(isoDate(d))} /></PopoverContent></Popover><Popover><PopoverTrigger asChild><Button variant="outline" className="h-9 rounded-lg"><CalendarDays className="h-4 w-4" />{t.to}: {toDate || "—"}</Button></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={parseIsoDate(toDate)} onSelect={(d) => setToDate(isoDate(d))} /></PopoverContent></Popover><Select value={sort} onValueChange={setSort}><SelectTrigger className="h-10 w-[170px] rounded-xl"><ArrowUpDown className="h-4 w-4" /><SelectValue /></SelectTrigger><SelectContent><SelectItem value="newest">{t.newest}</SelectItem><SelectItem value="oldest">{t.oldest}</SelectItem><SelectItem value="number">{t.numberSort}</SelectItem><SelectItem value="company">{t.companySort}</SelectItem></SelectContent></Select><Button variant="outline" onClick={reset} className="h-9 rounded-lg"><RotateCcw className="h-4 w-4" />{t.reset}</Button><Button variant="outline" onClick={exportExcel} className="h-9 rounded-lg" disabled={!filtered.length}><FileSpreadsheet className="h-4 w-4" />{t.export}</Button><Button variant="outline" onClick={() => { const opened = openPrintHtmlReport(document.documentElement.outerHTML); if (!opened) toast.error(locale === "ar" ? "تعذر فتح نافذة الطباعة." : "Could not open the print window."); }} className="h-9 rounded-lg" disabled={!filtered.length}><Printer className="h-4 w-4" />{t.print}</Button></div></CardContent></Card><Card className="rounded-lg border bg-card shadow-none"><CardHeader><CardTitle>{mode === "invoices" ? t.invoicesTitle : mode === "receipts" ? t.receiptsTitle : t.title}</CardTitle><CardDescription>{filtered.length} / {scoped.length}</CardDescription></CardHeader><CardContent>{filtered.length ? <DocumentTable rows={filtered} locale={locale} /> : <div className="py-16 text-center text-sm text-muted-foreground"><Search className="mx-auto mb-3 h-7 w-7" />{scoped.length ? t.noResults : t.noData}</div>}</CardContent></Card></>;
}
function KeyValue({ label, children }: { label: string; children: React.ReactNode }) {
  return <div className="rounded-lg border bg-background p-4"><p className="text-xs text-muted-foreground">{label}</p><div className="mt-1 break-words text-sm font-medium">{children}</div></div>;
}
...
function Detail({ document, receipts, locale }: { document: BillingDocument; receipts: BillingDocument[]; locale: Locale }) {
  ...
  const openBackend = (suffix: "print" | "pdf") => { const anchor = window.document.createElement("a"); anchor.href = apiUrl(`${API}${encodeURIComponent(document.id)}/${suffix}/`); anchor.target = "_blank"; anchor.rel = "noopener noreferrer"; window.document.body.appendChild(anchor); anchor.click(); anchor.remove(); };
  return <><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><Kpi title={t.documentNumber} value={document.number} /><Kpi title={t.status} value={<StatusBadge value={document.status} />} /><Kpi title={t.documentType} value={documentTypeLabel(document.type, locale)} /><Kpi title={t.total} value={<Money amount={document.total} currency={document.currency} />} /></div><div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_360px]"><div className="space-y-6"><Card className="rounded-lg border bg-card shadow-none"><CardHeader><CardTitle>{t.identity}</CardTitle></CardHeader><CardContent className="grid gap-4 md:grid-cols-2"><KeyValue label={t.documentNumber}><span className="font-mono text-xs">{document.number}</span></KeyValue><KeyValue label={t.issueDate}><span dir="ltr">{formatDate(document.issueDate)}</span></KeyValue><KeyValue label={t.paidAt}><span dir="ltr">{formatDate(document.paidAt, true)}</span></KeyValue><KeyValue label={t.paymentMethod}>{paymentMethodLabel(document.paymentMethod, locale)}</KeyValue><KeyValue label={t.billingReference}>{document.billingReference || "—"}</KeyValue><KeyValue label={t.transactionReference}>{document.transactionReference || "—"}</KeyValue>{document.cancelledAt ? <KeyValue label={t.cancelledAt}><span dir="ltr">{formatDate(document.cancelledAt, true)}</span></KeyValue> : null}{document.cancellationReason ? <KeyValue label={t.cancellationReason}>{document.cancellationReason}</KeyValue> : null}{document.notes ? <KeyValue label={t.notes}>{document.notes}</KeyValue> : null}</CardContent></Card><Card className="rounded-lg border bg-card shadow-none"><CardHeader><CardTitle>{t.financials}</CardTitle></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{[[t.subtotal, document.subtotal], [t.discount, document.discount], [t.taxable, document.taxable], [t.tax, document.tax], [t.total, document.total], [t.paid, document.paid], [t.balance, document.balance]].map(([label, amount]) => <KeyValue key={label} label={label}><Money amount={amount} currency={document.currency} /></KeyValue>)}</CardContent></Card><div className="grid gap-6 xl:grid-cols-2"><SnapshotCard title={t.seller} snapshot={document.seller} locale={locale} /><SnapshotCard title={t.buyer} snapshot={document.buyer} locale={locale} /></div><Card className="rounded-lg border bg-card shadow-none"><CardHeader><CardTitle>{t.immutable}</CardTitle><CardDescription>{t.safeSnapshot}</CardDescription></CardHeader><CardContent className="grid gap-4 md:grid-cols-2"><KeyValue label={t.plan}>{safeSnapshotValue(document.planSnapshot, "name", "plan_name", "title")}</KeyValue><KeyValue label={t.billingCycle}>{safeSnapshotValue(document.subscriptionSnapshot, "billing_cycle", "cycle")}</KeyValue><KeyValue label={t.action}>{safeSnapshotValue(document.subscriptionSnapshot, "action", "subscription_action")}</KeyValue><KeyValue label={t.billingReference}>{safeSnapshotValue(document.subscriptionSnapshot, "billing_reference", "reference")}</KeyValue></CardContent></Card>{document.type === "SUBSCRIPTION_INVOICE" ? <Card className="rounded-lg border bg-card shadow-none"><CardHeader><CardTitle>{t.linkedReceipts}</CardTitle><CardDescription>{receipts.length}</CardDescription></CardHeader><CardContent>{receipts.length ? <DocumentTable rows={receipts} locale={locale} /> : <p className="rounded-lg border bg-muted/20 p-4 text-sm text-muted-foreground">{t.none}</p>}</CardContent></Card> : null}<Card className="overflow-hidden rounded-lg border bg-card shadow-none"><CardHeader><CardTitle>{t.printable}</CardTitle><CardDescription>{t.printableDesc}</CardDescription></CardHeader><CardContent>{document.allowedPrint || document.printable ? <iframe title={`${t.printable} ${document.number}`} src={previewUrl} className="h-[760px] w-full rounded-2xl border bg-white" /> : <div className="rounded-lg border bg-muted/20 p-8 text-center text-sm text-muted-foreground">{t.previewUnavailable}</div>}</CardContent></Card></div><aside className="space-y-6"><Card className="rounded-lg border bg-card shadow-none"><CardHeader><CardTitle>{t.links}</CardTitle></CardHeader><CardContent className="grid gap-2">{document.companyId ? <Button asChild variant="outline" className="justify-start"><Link href={`/system/companies/${document.companyId}`}><Building2 className="h-4 w-4" />{document.companyName}</Link></Button> : null}{document.subscriptionId ? <Button asChild variant="outline" className="justify-start"><Link href={`/system/subscriptions/${document.subscriptionId}`}><ShieldCheck className="h-4 w-4" />{t.subscription} #{document.subscriptionId}</Link></Button> : null}{document.relatedInvoiceId ? <Button asChild variant="outline" className="justify-start"><Link href={`/system/invoices/${document.relatedInvoiceId}`}><FileText className="h-4 w-4" />{document.relatedInvoiceNumber || `#${document.relatedInvoiceId}`}</Link></Button> : null}<Button asChild variant="outline" className="justify-start"><Link href="/system/platform-payments/list"><CreditCard className="h-4 w-4" />{t.payments}</Link></Button></CardContent></Card><Card className="rounded-2xl border-primary/20 shadow-sm"><CardHeader><CardTitle>{t.printable}</CardTitle></CardHeader><CardContent className="grid gap-2"><Button variant="outline" className="justify-start" disabled={!document.allowedPrint} onClick={() => openBackend("print")}><Printer className="h-4 w-4" />{t.print}</Button><Button variant="outline" className="justify-start" disabled={!document.allowedPrint} onClick={() => openBackend("pdf")}><Download className="h-4 w-4" />{t.pdf}</Button></CardContent></Card><Card className="rounded-lg border bg-card shadow-none xl:sticky xl:top-6"><CardContent className="grid gap-2 pt-6"><Button asChild variant="outline" className="justify-start"><Link href="/system/invoices/list"><ListChecks className="h-4 w-4" />{t.invoices}</Link></Button><Button asChild variant="outline" className="justify-start"><Link href="/system/invoices/receipts"><ReceiptText className="h-4 w-4" />{t.receipts}</Link></Button><Button asChild variant="outline" className="justify-start"><Link href="/system"><LayoutDashboard className="h-4 w-4" />{t.dashboard}</Link></Button></CardContent></Card></aside></div></>;
}
