...
@csrf_protect
@require_POST
def system_subscription_suspend(
    request: HttpRequest,
    subscription_id: int,
) -> JsonResponse:
    if not user_has_system_permission(
        ...
        return JsonResponse(
            {
                "ok": False,
                "message": "غير مصرح لك بتعليق اشتراكات الشركات.",
    ...
    if subscription.status not in {
        ...
        return JsonResponse(
            {
                "ok": False,
                "message": "لا يمكن تعليق الاشتراك من حالته الحالية.",
                "errors": {
                    "status": (
                        "يجب أن يكون الاشتراك TRIAL أو ACTIVE "
                        "حتى يمكن تعليقه."
                    ),
                },
            },
    ...
    if reason:
        ...
        note = f"Suspension reason: {reason}"
        subscription.notes = (
            f"{current_notes}\n{note}".strip()
            if current_notes
            else note
