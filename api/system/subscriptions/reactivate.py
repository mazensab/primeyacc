...
@csrf_protect
@require_POST
def system_subscription_reactivate(request, subscription_id):
        ...
        return JsonResponse(
            {
                "ok": False,
                "message": "?? ???? ????? ????? ???????? ??? ??? ??? ?????? ??????.",
                "errors": {"status": "Subscription must be suspended to reactivate."},
            },
            status=400,
        )
    ...
    if reason:
        ...
        subscription.notes = f"{current_notes}\nReactivation reason: {reason}".strip()
        subscription.save(update_fields=["notes", "updated_at"])
    try:
        subscription.activate(paid_at=subscription.paid_at or timezone.now(), save=True)
