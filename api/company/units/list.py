...
# - هذا الملف مسؤول عن عرض قائمة الوحدات فقط
# - صلاحية العرض المطلوبة: company.units.view
# ============================================================
...
def company_units_list(request: Request) -> Response:
    """
    List catalog units for the current company only.
    ...
    try:
        ...
        queryset = get_company_units_queryset(company=company)
        queryset = filter_units_queryset(
            queryset,
            search=search,
            status=status,
        ...
        return Response(
            {
                "ok": True,
                "success": True,
                "message": "Catalog units loaded successfully.",
                "company": {
                    "id": company.id,
...
company_units_list.required_company_permissions = [
    "company.units.view",
]