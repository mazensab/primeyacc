# ============================================================
# ?? api/company/hr/payroll/runs/create.py
# ?? PrimeyAcc | Payroll Run Create API
# ============================================================

from __future__ import annotations

from django.core.exceptions import ValidationError
from django.db import IntegrityError
from rest_framework.decorators import api_view, permission_classes
from rest_framework.response import Response

from api.permissions import HasAnyCompanyPermission
from hr.services import create_payroll_run

from .serializers import (
    build_payroll_run_data_from_request,
    resolve_company_payroll_period,
    serialize_payroll_run,
)


@api_view(["POST"])
@permission_classes([HasAnyCompanyPermission])
def payroll_run_create(request):
    company = getattr(request, "company", None)

    if not company:
        return Response(
            {
                "ok": False,
                "success": False,
                "message": "Active company context is required.",
            },
            status=401,
        )

    try:
        period = resolve_company_payroll_period(
            company,
            request.data.get("period_id"),
        )
        data = build_payroll_run_data_from_request(request)
        payroll_run = create_payroll_run(
            company=company,
            period=period,
            created_by=request.user,
            data=data,
        )

    except ValidationError as exc:
        return Response(
            {
                "ok": False,
                "success": False,
                "message": "Invalid payroll run data.",
                "errors": exc.message_dict if hasattr(exc, "message_dict") else exc.messages,
            },
            status=400,
        )

    except IntegrityError:
        return Response(
            {
                "ok": False,
                "success": False,
                "message": "Payroll run already exists for this period or run number.",
            },
            status=400,
        )

    return Response(
        {
            "ok": True,
            "success": True,
            "message": "Payroll run created successfully.",
            "payroll_run": serialize_payroll_run(payroll_run),
        },
        status=201,
    )


payroll_run_create.required_company_permissions = [
    "company.hr.payroll.runs.create",
]
