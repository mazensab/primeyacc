...
from inventory.services import post_physical_inventory_count
...
def company_physical_inventory_count_post(
    request: Request,
    count_id: int,
) -> Response:
    try:
        ...
        count = post_physical_inventory_count(
            company=company,
            count=count,
            user=user,
            post_accounting=payload.get("post_accounting", True),
        )
        ...
        return Response(
            {
                "ok": True,
                "success": True,
                "message": "Physical inventory count posted successfully.",
                "physical_count": data,
                "data": data,
            },
            status=200,
        ...
        return Response(
            {
                "ok": False,
                "success": False,
                "message": "Physical inventory count could not be posted.",
...
company_physical_inventory_count_post.required_company_permissions = [
