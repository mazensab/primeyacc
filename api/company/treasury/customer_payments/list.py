...
# 🧠 PrimeyAcc | Company Treasury Customer Payments List/Create API V1.0
# ------------------------------------------------------------
...
def serialize_customer_payment(payment: CustomerPayment) -> dict[str, Any]:
    ...
    return {
        ...
        "sales_invoice_number": getattr(payment.sales_invoice, "invoice_number", ""),
...
def _paginate(queryset: QuerySet[CustomerPayment], request: Request) -> tuple[list[CustomerPayment], dict[str, Any]]:
    try:
