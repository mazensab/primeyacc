...
R=Path.cwd();OUT=R/"v2_26_b2c_postsplit_enrichment_exact_plan_v4.txt"
...
print("===== PRIMEYACC V2-26 B2C POST-SPLIT PLAN V4 TARGETED =====",flush=True)
...
maps=list(LegacyObjectMap.objects.filter(source_system='mhamcloud_v1',source_table__in=['business','business_locations','branches','locations']).values('company_id','source_table','legacy_id','legacy_company_id'))
...
companies={c.id:c for c in Company.objects.filter(id__in=ids).only('id','company_code','name','commercial_registration','tax_number','phone','mobile','city','address')}
