...
R=Path.cwd();P=R/"primey_frontend_v2/app/system/companies/[id]/page.tsx";D=R/"api/system/companies/detail.py";REP=R/"v2_26_b2c_company11_page_pilot_v1.txt"
...
try:
 ...
 if rc and not ("app/company/sales/components/company-sales-detail.tsx(35,386): error TS2322" in o and "app/system/companies/[id]/page.tsx" not in o):raise RuntimeError("target tsc")
 REP.write_text("\n".join(logs)+f"\nDETAIL_SHA={sha(D)}\nPAGE_SHA={sha(P)}\nRESULT=PASS\n",encoding="utf8")
 print("===== PRIMEYACC V2-26 B2C COMPANY11 PAGE PILOT V1 =====");print(f"REPORT={REP.name}");print(f"DETAIL_SHA={sha(D)}");print(f"PAGE_SHA={sha(P)}");print("TARGET_TSC=PASS");print("DJANGO_CHECK=PASS");print("MIGRATION_DRIFT=PASS");print("RESULT=PASS")
