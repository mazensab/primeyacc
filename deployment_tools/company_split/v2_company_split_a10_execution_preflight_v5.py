...
REPORT = ROOT / "v2_company_split_a10_execution_preflight_v5.txt"
...
def find_non_json_path(value, path="$"):
    scalar = (str, int, float, bool, type(None))
    if isinstance(value, scalar):
        return None
    if isinstance(value, dict):
        for key, item in value.items():
            problem = find_non_json_path(item, f"{path}.{key}")
            if problem:
                return problem
        return None
    if isinstance(value, (list, tuple)):
        for idx, item in enumerate(value):
            problem = find_non_json_path(item, f"{path}[{idx}]")
            if problem:
                return problem
        return None
    return f"{path} -> {type(value).__module__}.{type(value).__name__}: {value!r}"
...
def main() -> int:
    lines = [
        "=" * 120,
        "PRIMEYACC — COMPANY SPLIT A10 V5 MUTATION PREFLIGHT & EXACT EXECUTION PLAN",
        "=" * 120,
        f"ROOT={ROOT}",
    ...
    try:
        ...
        if manifest.get("schema") != "primeyacc.company_split_transformation_manifest.v2":
            raise RuntimeError(
                f"Expected Manifest v2 schema, got {manifest.get('schema')}"
            )
        if int(
            (manifest.get("usage_master_contract") or {}).get(
                "estimated_new_clones_local_snapshot", -1
            )
        ) != 43891:
            raise RuntimeError(
                "Manifest v2 usage-master clone estimate must be 43891"
            )

        payment_party_contract = (
            (manifest.get("customer_payment_contract") or {})
            .get("business_party_coverage_contract")
            or {}
        )
        if int(payment_party_contract.get("additional_clone_action_count", -1)) != 4:
            raise RuntimeError(
                "Manifest v2 payment-driven BusinessParty clone count must be 4"
            )
        ...
        if git_branch != "main":
            raise RuntimeError(f"Expected main, got {git_branch}")
        ...
        # Resolve the authoritative Manifest-v2 CustomerPayment -> BusinessParty
        # coverage rows from legacy contact identities to current local Party IDs.
        # Current IDs remain local snapshot only; production replay resolves again.
        BusinessPartyPreflight = apps.get_model("parties", "BusinessParty")
        bp_ct = apps.get_model(
            "contenttypes", "ContentType"
        ).objects.get_for_model(BusinessPartyPreflight)

        payment_party_manifest_groups = defaultdict(set)
        payment_party_manifest_source_coverage = defaultdict(set)
        payment_party_identity_errors = []
        ...
        for row in payment_party_rows:
            ...
            if len(maps) != 1:
                payment_party_identity_errors.append(
                    f"company={lcid} contact={legacy_contact_id} maps={len(maps)}"
                )
                continue

            raw_target = clean(maps[0].target_object_id)
            if not raw_target.isdigit():
                payment_party_identity_errors.append(
                    f"company={lcid} contact={legacy_contact_id} invalid_target={raw_target}"
                )
                continue

            party_id = int(raw_target)
            party = BusinessPartyPreflight.objects.filter(
                pk=party_id,
                company_id=company_id_by_legacy[lcid],
            ).first()

            if party is None:
                payment_party_identity_errors.append(
                    f"company={lcid} contact={legacy_contact_id} "
                    f"party={party_id} missing_or_wrong_company"
                )
                continue

            for gid in row.get("payment_usage_coverage_groups", []) or []:
                gid = clean(gid)
                if gid:
                    payment_party_manifest_groups[(lcid, party_id)].add(gid)

            for gid in row.get("source_group_coverage_no_clone", []) or []:
                gid = clean(gid)
                if gid:
                    payment_party_manifest_source_coverage[(lcid, party_id)].add(gid)

        if payment_party_identity_errors:
            raise RuntimeError(
                "Manifest-v2 CustomerPayment BusinessParty identity resolution failed: "
                f"{payment_party_identity_errors}"
            )

        print(
            "[A10]   manifest-v2 payment-driven BusinessParty coverage "
            f"rows={len(payment_party_rows)}",
            flush=True,
        )
        ...
        for app_label, model_name in USAGE_MASTER_MODELS:
            ...
            # Manifest v2 extends BusinessParty usage with raw party references
            # from CustomerPayment. These are not Django FKs, so they cannot be
            # discovered by relation_fields_to() and must be merged explicitly.
            if model_label(master_model) == "parties.BusinessParty":
                for key, groups in payment_party_manifest_groups.items():
                    target_groups_by_master[key].update(groups)
        ...
        manifest_party_coverage_errors = []
        resolved_party_groups = usage_master_group_sets.get(
            "parties.BusinessParty", {}
        )
        for key, required_groups in payment_party_manifest_groups.items():
            actual_groups = resolved_party_groups.get(key, set())
            missing = sorted(set(required_groups) - set(actual_groups))
            if missing:
                manifest_party_coverage_errors.append(
                    f"key={key} missing_groups={missing}"
                )

        if manifest_party_coverage_errors:
            raise RuntimeError(
                "Manifest-v2 payment-driven BusinessParty coverage did not "
                "survive usage-master fingerprinting: "
                f"{manifest_party_coverage_errors[:20]}"
            )
        ...
        plan = {
            ...
            "manifest_v2_payment_party_coverage": {
                "authoritative_manifest_sha256": EXPECTED_MANIFEST_SHA256,
                "coverage_row_count": len(payment_party_rows),
                "resolved_current_party_count_snapshot": len(
                    payment_party_manifest_groups
                ),
                "additional_clone_action_count": int(
                    payment_party_contract.get(
                        "additional_clone_action_count", 0
                    )
                ),
                "source_group_coverage_no_clone_count": int(
                    payment_party_contract.get(
                        "source_group_coverage_no_clone_count", 0
                    )
                ),
            },
        ...
        non_json_path = find_non_json_path(plan)
        if non_json_path:
            raise RuntimeError(
                f"Execution plan contains non-JSON value: {non_json_path}"
            )
        ...
        if total_usage_clone_actions != 43891:
            safety_errors.append(
                f"Manifest-v2 fixed usage-master expectation=43891 actual={total_usage_clone_actions}"
            )
        ...
        lines.extend([
            ...
            f"MANIFEST_V2_PAYMENT_PARTY_ROW_COUNT={len(payment_party_rows)}",
            f"MANIFEST_V2_PAYMENT_PARTY_RESOLVED_COUNT={len(payment_party_manifest_groups)}",
            f"MANIFEST_V2_PAYMENT_PARTY_ADDITIONAL_CLONES={payment_party_contract.get('additional_clone_action_count')}",
        ...
        print("===== PRIMEYACC COMPANY SPLIT A10 V5 =====")
        ...
        print("===== PRIMEYACC COMPANY SPLIT A10 V5 =====", flush=True)
