...
REPORT=ROOT/'v2_company_split_final_one_shot_v2.txt'
SNAP=ROOT/'v2_company_split_final_apply_snapshot_v2.json'
SRC='mhamcloud_v1'
SHAS={
...
def build_m3(m2,pol,special_routes):
    ...
    m['manifest_lineage']=dict(
        ...
        special_customer_payment_route_identity='LEGACY_COMPANY_ID+LEGACY_PAYMENT_ID',
    )
    ...
    cp['special_legacy_payment_route_identity']='LEGACY_COMPANY_ID+LEGACY_PAYMENT_ID'
...
def stable_special_routes(M,C):
    ...
    for current_payment_id,(target_group,current_branch_id) in sorted(C['special'].items()):
        ...
        if len(maps)!=1:
            raise RuntimeError(
                f'special payment legacy identity unresolved '
                f'company={lcid} current_payment={current_payment_id} maps={len(maps)}'
            )
        legacy_payment_id=c(maps[0].legacy_id)
...
def restore_stash():
    ...
    return len(expected)
def main():
    a=args(); committed=False; lines=['='*120,'PRIMEYACC — COMPANY SPLIT FINAL ONE-SHOT V2','='*120,f'GENERATED_AT_UTC={datetime.now(timezone.utc).isoformat()}',f'ROOT={ROOT}',f'APPLY_AFTER_REHEARSAL={"YES" if a.apply_after_rehearsal else "NO"}',f'RESTORE_V2_26C={"YES" if a.restore_v2_26c else "NO"}','GIT_PUSH=0','']
    try:
        ...
        static_checks();lines += [
            ...
            'SPECIAL_PAYMENT_ROUTE_IDENTITY=LEGACY_COMPANY_ID+LEGACY_PAYMENT_ID',
        ...
        if not a.apply_after_rehearsal:
            lines += ['PERMANENT_APPLY=SKIPPED','MUTATION_READY=YES','FINAL_RESULT=PASS_REHEARSAL_ONLY','='*120];REPORT.write_text('\n'.join(lines)+'\n',encoding='utf-8');print('===== PRIMEYACC COMPANY SPLIT FINAL ONE-SHOT V2 =====');print('FINAL_RESULT=PASS_REHEARSAL_ONLY');print('MUTATION_READY=YES');print('DATABASE_PERMANENT_WRITES=0');print(f'MANIFEST_V3_SHA256={m3sha}');print(f'REPORT={REPORT.name}');print(f'REPORT_SHA256={sha(REPORT)}');return 0
