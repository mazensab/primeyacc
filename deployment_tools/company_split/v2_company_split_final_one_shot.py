...
REPORT=ROOT/'v2_company_split_final_one_shot.txt'
SNAP=ROOT/'v2_company_split_final_apply_snapshot.json'
SRC='mhamcloud_v1'
SHAS={
...
def build_m3(m2,pol):
    ...
    m['manifest_lineage']=dict(m.get('manifest_lineage') or {},base_manifest_v2_sha256=SHAS['m2'],a10e_report_sha256=SHAS['a10er'],a10e_opening_balance_policy_sha256=SHAS['a10ep'])
    m['business_party_financial_policy']={k:v for k,v in pol.items() if k not in {'current_business_party_id_snapshot','supersedes_a10d_policy_sha256','manifest_v2_sha256','schema'}}
    return m
...
def payments(M,C,T,F,U):
    CP,BP=M['CustomerPayment'],M['BusinessParty']; rev={v:k for k,v in C['company_ids'].items()}; ids=list(rev); bpcompany=dict(BP.objects.filter(company_id__in=ids).values_list('id','company_id')); moved=sp=0;part=[];full=[];batch=[]
...
def transform(M,C,manifest,plan,base,rehearsal):
    ...
    stats={'full_excluded_subscription_rows':M['CompanySubscription'].objects.filter(company_id__in=C['fullc']).count()};T=make_targets(M,C,manifest,rehearsal,neg);F=foundation(M,C,T,manifest,rehearsal,neg);U=usage_clone(M,C,T,F,manifest,rehearsal,neg);stats['membership_clones'],stats['membership_deletes']=memberships(M,C,T,plan,rehearsal,neg);profiles(M,T,plan);subscriptions(M,T,plan,rehearsal,neg);reparent(M,C,T,manifest);stats['ops']=ops(M,C,T,U);stats['payments_moved'],part,full=payments(M,C,T,F,U);stats['maps_moved']=move_maps(M,C);stats['partial']=purge_partial(M,C,part);stats['full_companies']=purge_full(M,C);stats['users_deleted'],stats['users_kept']=users(M,plan);verify(M,C,T,manifest,plan,base,stats);return stats
...
def restore_stash():
    ...
    return len(expected)
def main():
    a=args(); committed=False; lines=['='*120,'PRIMEYACC — COMPANY SPLIT FINAL ONE-SHOT','='*120,f'GENERATED_AT_UTC={datetime.now(timezone.utc).isoformat()}',f'ROOT={ROOT}',f'APPLY_AFTER_REHEARSAL={"YES" if a.apply_after_rehearsal else "NO"}',f'RESTORE_V2_26C={"YES" if a.restore_v2_26c else "NO"}','GIT_PUSH=0','']
    try:
        ...
        for tok in ['A10E_RESULT=PASS','BALANCE_OWNER_GROUP=88-G01','CLONE_88-G02_OPENING_BALANCE=0.00','NO_DUPLICATION_CHECK=PASS']:
        ...
        m2=json.loads(M2.read_text(encoding='utf-8'));pol=json.loads(A10EP.read_text(encoding='utf-8'));manifest=build_m3(m2,pol);M3.write_text(json.dumps(manifest,ensure_ascii=False,indent=2,sort_keys=True)+'\n',encoding='utf-8');m3sha=sha(M3);plan=json.loads(PLANF.read_text(encoding='utf-8'));M=load_models();
        if M['Company'].objects.filter(extra_data__primey_company_split__manifest='v3').exists():raise RuntimeError('Company split v3 already appears applied; do not rerun this one-shot')
        C=context(M,manifest,plan);base=baseline(M,C)
        ...
        static_checks();lines += ['===== FINAL PREFLIGHT =====',f'MANIFEST_V3_SHA256={m3sha}',f'USAGE_MASTER_CLONES={uc}',f'OPERATIONAL_ROUTE_ROWS={route_total}',f'CUSTOMER_PAYMENT_ROWS={cp}','FINANCIAL_POLICY=88-G01:24.00;88-G02:0.00','FINAL_PREFLIGHT=PASS','']
        ...
        restore(M,C,base);lines += ['===== ROLLBACK REHEARSAL =====','REHEARSAL_TRANSFORM=PASS','REHEARSAL_VERIFY=PASS','REHEARSAL_ROLLBACK=PASS','BASELINE_RESTORED=PASS',f'REHEARSAL_STATS_SHA256={hashlib.sha256(json.dumps(rs,sort_keys=True,default=str).encode()).hexdigest().upper()}','']
        if not a.apply_after_rehearsal:
            lines += ['PERMANENT_APPLY=SKIPPED','MUTATION_READY=YES','FINAL_RESULT=PASS_REHEARSAL_ONLY','='*120];REPORT.write_text('\n'.join(lines)+'\n',encoding='utf-8');print('===== PRIMEYACC COMPANY SPLIT FINAL ONE-SHOT =====');print('FINAL_RESULT=PASS_REHEARSAL_ONLY');print('MUTATION_READY=YES');print('DATABASE_PERMANENT_WRITES=0');print(f'MANIFEST_V3_SHA256={m3sha}');print(f'REPORT={REPORT.name}');print(f'REPORT_SHA256={sha(REPORT)}');return 0
        ...
        print('[FINAL] 7/7 Writing final closure report...',flush=True);lines += ['===== FINAL CLOSURE =====',f'HEAD={git("rev-parse","HEAD")}',f'ORIGIN_MAIN={git("rev-parse","origin/main")}',f'TRACKED_STATUS_AFTER={c(git("status","--short","--untracked-files=no"))}','GIT_PUSH=0','DATABASE_TRANSFORMATION=COMMITTED','LOCAL_COMPANY_SPLIT=FINAL_CLOSED','PRODUCTION_CUTOVER_CONTRACT=MANIFEST_V3','FINAL_RESULT=PASS_APPLIED','='*120];REPORT.write_text('\n'.join(lines)+'\n',encoding='utf-8');print('===== PRIMEYACC COMPANY SPLIT FINAL ONE-SHOT =====');print('FINAL_RESULT=PASS_APPLIED');print('LOCAL_COMPANY_SPLIT=FINAL_CLOSED');print(f'MANIFEST_V3_SHA256={m3sha}');print(f'GLOBAL_USERS_DELETED={len(astats["users_deleted"])}');print(f'GLOBAL_USERS_KEPT_DUE_DEPENDENCIES={len(astats["users_kept"])}');print(f'V2_26C_RESTORED={"YES" if a.restore_v2_26c else "NO"}');print('GIT_PUSH=0');print(f'REPORT={REPORT.name}');print(f'REPORT_SHA256={sha(REPORT)}');print(f'APPLY_SNAPSHOT_SHA256={sha(SNAP)}');return 0
