...
REPORT=ROOT/'v2_company_split_final_one_shot_v10.txt'
SNAP=ROOT/'v2_company_split_final_apply_snapshot_v10.json'
SRC='mhamcloud_v1'
SHAS={
...
def purge_full(M,C):
    ...
    progress(
        f'[TX] full exclusions purge: DEPENDENCY_SAFE start companies={len(ids)}'
    )

    # Canonical source maps for fully excluded Companies must not survive locally.
    ...
    def company_fk_contracts(model):
        ...
        for field,target in fks(model):
            ...
            delete_name=c(getattr(delete_fn,'__name__',str(delete_fn)))
            out.append((field,delete_name))
        return out

    def owned_company_ids(obj):
        contracts=company_fk_contracts(obj.__class__)
        owned=[]
        reference_only=[]
        for field,delete_name in contracts:
            ...
            if delete_name in {'CASCADE','PROTECT','RESTRICT'}:
                ...
                reference_only.append((field.name,delete_name,int(value)))
    ...
    def prove_same_company(obj,cid,seen=None,depth=0):
        ...
        # A direct owned Company contract is authoritative. Never delete a
        # protected object whose owned Company differs from the excluded Company.
        if owned:
            return all(x==cid for x in owned)

        # A pure SET_NULL/SET_DEFAULT/DO_NOTHING Company reference is not proof
        # of ownership; do not delete the object based on that alone.
        if reference_only:
            return False

        # Some dependent detail models do not carry company_id directly.
        # Prove ownership through a parent object that itself belongs to cid.
    ...
    def flatten_restricted(value):
        ...
        if isinstance(value,(list,tuple,set,frozenset)):
            ...
            return rows
        try:
            # QuerySet / iterable collections, but not model instances.
    ...
    def safe_delete_queryset(model,qs,cid,reason):
        ...
        if stack_key in delete_stack:
            raise RuntimeError(
                f'Protected-delete dependency cycle '
                f'company={cid} stack={delete_stack+[stack_key]}'
            )

        delete_stack.append(stack_key)
        try:
            while True:
                pks=list(
                ...
                try:
                    ...
                    for obj in protected:
                        if not prove_same_company(obj,cid):
                            raise RuntimeError(
                                f'CROSS_COMPANY_OR_UNPROVEN_PROTECTED_OBJECT '
                                f'company={cid} deleting={label} '
                                f'protected={mlabel(obj.__class__)}:{obj.pk}'
                            ) from exc
                        grouped[obj.__class__].append(obj.pk)
                    ...
                    for obj in restricted:
                        ...
                        if not prove_same_company(obj,cid):
                            raise RuntimeError(
                                f'CROSS_COMPANY_OR_UNPROVEN_RESTRICTED_OBJECT '
                                f'company={cid} deleting={label} '
    ...
    def owned_models_for_company(cid):
        ...
        for model in M['apps'].get_models():
            ...
            for field,delete_name in company_fk_contracts(model):
                lookup={field.attname:cid}
                count=model._default_manager.filter(**lookup).count()
                if not count:
                    continue

                if delete_name in {'CASCADE','PROTECT','RESTRICT'}:
                    rows.append((model,field,delete_name,count))
                elif delete_name=='DO_NOTHING':
                    do_nothing_blockers.append(
                        f'{mlabel(model)}.{field.name}={count}'
                    )
                # SET_NULL / SET_DEFAULT are reference-only and are intentionally
                # left for Company.delete() semantics.

        if do_nothing_blockers:
            raise RuntimeError(
                f'FULL_EXCLUSION_DO_NOTHING_BLOCKERS '
                f'company={cid} blockers={do_nothing_blockers}'
            )

        # Operational/detail rows first by larger dependency surface/count,
        # while recursive PROTECT handling guarantees correctness.
        rows.sort(
            key=lambda x:(
    ...
    for index,cid in enumerate(ids,1):
        ...
        for model,field,delete_name,count in contracts:
            # Re-evaluate because earlier deletions may already have cascaded rows.
            qs=model._default_manager.filter(**{field.attname:cid})
            remaining=qs.count()
            if not remaining:
                continue
            ...
            if after:
                raise RuntimeError(
                    f'Full exclusion owned rows remain '
                    f'company={cid} model={mlabel(model)} '
                    f'field={field.name} rows={after}'
                )
            summary[f'{mlabel(model)}.{field.name}']+=remaining

        progress(
            f'[TX] full exclusion company={cid} '
            f'owned rows cleared; deleting Company row'
        )

        try:
            company.delete()
        except (ProtectedError,RestrictedError) as exc:
            # At this point any remaining blocker was not represented as a
            # direct owned Company contract and needs explicit inspection.
            ...
            raise RuntimeError(
                f'FULL_EXCLUSION_UNCOVERED_DEPENDENCY '
...
def restore_stash():
    ...
    return len(expected)
def main():
    a=args(); committed=False; lines=['='*120,'PRIMEYACC — COMPANY SPLIT FINAL ONE-SHOT V10','='*120,f'GENERATED_AT_UTC={datetime.now(timezone.utc).isoformat()}',f'ROOT={ROOT}',f'APPLY_AFTER_REHEARSAL={"YES" if a.apply_after_rehearsal else "NO"}',f'RESTORE_V2_26C={"YES" if a.restore_v2_26c else "NO"}','GIT_PUSH=0','']
    try:
        ...
        baseline_hash=hashlib.sha256(
            json.dumps(base,sort_keys=True,default=str).encode()
        ).hexdigest().upper()
        progress(f"[FINAL] baseline guard hash={baseline_hash}")
        if baseline_hash!='9B47BF2F6CF89C8F1E6353E81BAEDB8109E3FE1527C96C22DBE06BF188E4B3E3':
            raise RuntimeError(
                f'Baseline hash drift after interrupted rehearsal '
                f'expected=9B47BF2F6CF89C8F1E6353E81BAEDB8109E3FE1527C96C22DBE06BF188E4B3E3 actual={baseline_hash}'
            )
        ...
        progress('[FINAL] rehearsal PK strategy=MANUAL_POSITIVE_MAX_PLUS_ONE_NO_SEQUENCE_ADVANCE')
        ...
        restore(M,C,base);progress('[FINAL] exact baseline restoration PASS');lines += [
            '===== ROLLBACK REHEARSAL =====',
            'REHEARSAL_TEMP_PK_STRATEGY=MANUAL_POSITIVE_MAX_PLUS_ONE_NO_SEQUENCE_ADVANCE',
            'REHEARSAL_TRANSFORM=PASS',
            'REHEARSAL_VERIFY=PASS',
            'REHEARSAL_ROLLBACK=PASS',
            'BASELINE_RESTORED=PASS',
            f'REHEARSAL_STATS_SHA256={hashlib.sha256(json.dumps(rs,sort_keys=True,default=str).encode()).hexdigest().upper()}',
            ''
        ]
        if not a.apply_after_rehearsal:
            lines += ['PERMANENT_APPLY=SKIPPED','MUTATION_READY=YES','FINAL_RESULT=PASS_REHEARSAL_ONLY','='*120];REPORT.write_text('\n'.join(lines)+'\n',encoding='utf-8');print('===== PRIMEYACC COMPANY SPLIT FINAL ONE-SHOT V10 =====');print('FINAL_RESULT=PASS_REHEARSAL_ONLY');print('MUTATION_READY=YES');print('DATABASE_PERMANENT_WRITES=0');print(f'MANIFEST_V3_SHA256={m3sha}');print(f'REPORT={REPORT.name}');print(f'REPORT_SHA256={sha(REPORT)}');return 0
        ...
        print('[FINAL] 7/7 Writing final closure report...',flush=True);lines += ['===== FINAL CLOSURE =====',f'HEAD={git("rev-parse","HEAD")}',f'ORIGIN_MAIN={git("rev-parse","origin/main")}',f'TRACKED_STATUS_AFTER={c(git("status","--short","--untracked-files=no"))}','GIT_PUSH=0','DATABASE_TRANSFORMATION=COMMITTED','LOCAL_COMPANY_SPLIT=FINAL_CLOSED','PRODUCTION_CUTOVER_CONTRACT=MANIFEST_V3','FINAL_RESULT=PASS_APPLIED','='*120];REPORT.write_text('\n'.join(lines)+'\n',encoding='utf-8');print('===== PRIMEYACC COMPANY SPLIT FINAL ONE-SHOT V10 =====');print('FINAL_RESULT=PASS_APPLIED');print('LOCAL_COMPANY_SPLIT=FINAL_CLOSED');print(f'MANIFEST_V3_SHA256={m3sha}');print(f'GLOBAL_USERS_DELETED={len(astats["users_deleted"])}');print(f'GLOBAL_USERS_KEPT_DUE_DEPENDENCIES={len(astats["users_kept"])}');print(f'V2_26C_RESTORED={"YES" if a.restore_v2_26c else "NO"}');print('GIT_PUSH=0');print(f'REPORT={REPORT.name}');print(f'REPORT_SHA256={sha(REPORT)}');print(f'APPLY_SNAPSHOT_SHA256={sha(SNAP)}');return 0
