...
REPORT=ROOT/'v2_company_split_final_one_shot_v7.txt'
SNAP=ROOT/'v2_company_split_final_apply_snapshot_v7.json'
SRC='mhamcloud_v1'
SHAS={
...
def restore_stash():
    ...
    return len(expected)
def main():
    a=args(); committed=False; lines=['='*120,'PRIMEYACC — COMPANY SPLIT FINAL ONE-SHOT V7','='*120,f'GENERATED_AT_UTC={datetime.now(timezone.utc).isoformat()}',f'ROOT={ROOT}',f'APPLY_AFTER_REHEARSAL={"YES" if a.apply_after_rehearsal else "NO"}',f'RESTORE_V2_26C={"YES" if a.restore_v2_26c else "NO"}','GIT_PUSH=0','']
    try:
        ...
        if not a.apply_after_rehearsal:
            lines += ['PERMANENT_APPLY=SKIPPED','MUTATION_READY=YES','FINAL_RESULT=PASS_REHEARSAL_ONLY','='*120];REPORT.write_text('\n'.join(lines)+'\n',encoding='utf-8');print('===== PRIMEYACC COMPANY SPLIT FINAL ONE-SHOT V7 =====');print('FINAL_RESULT=PASS_REHEARSAL_ONLY');print('MUTATION_READY=YES');print('DATABASE_PERMANENT_WRITES=0');print(f'MANIFEST_V3_SHA256={m3sha}');print(f'REPORT={REPORT.name}');print(f'REPORT_SHA256={sha(REPORT)}');return 0
