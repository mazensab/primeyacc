...
REPORT=ROOT/'v2_company_split_final_one_shot_v6.txt'
SNAP=ROOT/'v2_company_split_final_apply_snapshot_v6.json'
SRC='mhamcloud_v1'
SHAS={
...
def memberships(M,C,T,plan,rehearsal,neg):
    ...
    for a in plan['membership_actions']:
        ...
        def setpol(pol,gid,spec):
            ...
            # Branch ownership must already match the membership Company.
            # This is why Branch reparenting runs before membership policies.
            membership_company_id=CM.objects.only('company_id').get(pk=pol.membership_id).company_id
            for bid in bids:
                branch_company_id=M['Branch'].objects.only('company_id').get(pk=bid).company_id
                if int(branch_company_id)!=int(membership_company_id):
                    raise RuntimeError(
                        f'BranchPolicy ownership mismatch membership={pol.membership_id} '
                        f'membership_company={membership_company_id} branch={bid} '
                        f'branch_company={branch_company_id} gid={gid}'
                    )
...
def restore_stash():
    ...
    return len(expected)
def main():
    a=args(); committed=False; lines=['='*120,'PRIMEYACC — COMPANY SPLIT FINAL ONE-SHOT V6','='*120,f'GENERATED_AT_UTC={datetime.now(timezone.utc).isoformat()}',f'ROOT={ROOT}',f'APPLY_AFTER_REHEARSAL={"YES" if a.apply_after_rehearsal else "NO"}',f'RESTORE_V2_26C={"YES" if a.restore_v2_26c else "NO"}','GIT_PUSH=0','']
    try:
        ...
        if not a.apply_after_rehearsal:
            lines += ['PERMANENT_APPLY=SKIPPED','MUTATION_READY=YES','FINAL_RESULT=PASS_REHEARSAL_ONLY','='*120];REPORT.write_text('\n'.join(lines)+'\n',encoding='utf-8');print('===== PRIMEYACC COMPANY SPLIT FINAL ONE-SHOT V6 =====');print('FINAL_RESULT=PASS_REHEARSAL_ONLY');print('MUTATION_READY=YES');print('DATABASE_PERMANENT_WRITES=0');print(f'MANIFEST_V3_SHA256={m3sha}');print(f'REPORT={REPORT.name}');print(f'REPORT_SHA256={sha(REPORT)}');return 0
        ...
        print('[FINAL] 7/7 Writing final closure report...',flush=True);lines += ['===== FINAL CLOSURE =====',f'HEAD={git("rev-parse","HEAD")}',f'ORIGIN_MAIN={git("rev-parse","origin/main")}',f'TRACKED_STATUS_AFTER={c(git("status","--short","--untracked-files=no"))}','GIT_PUSH=0','DATABASE_TRANSFORMATION=COMMITTED','LOCAL_COMPANY_SPLIT=FINAL_CLOSED','PRODUCTION_CUTOVER_CONTRACT=MANIFEST_V3','FINAL_RESULT=PASS_APPLIED','='*120];REPORT.write_text('\n'.join(lines)+'\n',encoding='utf-8');print('===== PRIMEYACC COMPANY SPLIT FINAL ONE-SHOT V6 =====');print('FINAL_RESULT=PASS_APPLIED');print('LOCAL_COMPANY_SPLIT=FINAL_CLOSED');print(f'MANIFEST_V3_SHA256={m3sha}');print(f'GLOBAL_USERS_DELETED={len(astats["users_deleted"])}');print(f'GLOBAL_USERS_KEPT_DUE_DEPENDENCIES={len(astats["users_kept"])}');print(f'V2_26C_RESTORED={"YES" if a.restore_v2_26c else "NO"}');print('GIT_PUSH=0');print(f'REPORT={REPORT.name}');print(f'REPORT_SHA256={sha(REPORT)}');print(f'APPLY_SNAPSHOT_SHA256={sha(SNAP)}');return 0
