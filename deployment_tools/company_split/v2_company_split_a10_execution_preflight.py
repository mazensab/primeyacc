...
REPORT = ROOT / "v2_company_split_a10_execution_preflight.txt"
PLAN = ROOT / "v2_company_split_local_execution_plan.json"
...
def main() -> int:
    lines = [
        "=" * 120,
        "PRIMEYACC — COMPANY SPLIT A10 MUTATION PREFLIGHT & EXACT EXECUTION PLAN",
        "=" * 120,
        f"ROOT={ROOT}",
    ...
    try:
        ...
        for model, branch_lookup in route_specs:
            ...
            for row in qs.iterator(chunk_size=50000):
                pk = int(row["pk"])
                cid = int(row[f"{company_field.name}_id"])
                lcid = legacy_by_company_id[cid]
                bid = row[branch_lookup]
        ...
        for payment in cp_qs.iterator(chunk_size=20000):
            ...
            if payment.sales_invoice_id and payment.sales_invoice and payment.sales_invoice.branch_id:
                ...
                if route is None:
                    raise RuntimeError(
                        f"CustomerPayment {payment.id} missing from frozen A6 route map"
                    )
        ...
        if no_invoice_or_no_branch != 412:
            raise RuntimeError(
                f"CustomerPayment no-invoice/no-branch drift expected=412 actual={no_invoice_or_no_branch}"
            )
        ...
        print("===== PRIMEYACC COMPANY SPLIT A10 =====")
        ...
        print("===== PRIMEYACC COMPANY SPLIT A10 =====", flush=True)
